...
void PrtManager::AddHit(PrtHit hit){
  ...
  if(fRunType==1 || fRunType==5 || fRunType==11){
	...
	std::cout<<"########## id "<<id<<"  Momentum phi "<<hit.GetMomentum().Phi()<<"  time= "<<time<<std::endl;
	
      if(fRunType==11) time -= fTime;
      
      ((PrtLutNode*)(fLut->At(id)))->AddEntry(id, hit.GetMomentum(), hit.GetPathInPrizm(), hit.GetNreflectionsInPrizm(), time, hit.GetLocalPos(),hit.GetDigiPos());
    }
  }
}
