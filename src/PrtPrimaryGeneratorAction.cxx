...
void PrtPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent){
    ...
    for (size_t i=0;i<store->size();i++){
        ...
        if((*store)[i]->GetName()=="wDirc") { //  Dirc Physical volum
            ...
            G4LogicalVolume * temp_0 = (*store)[i]->GetLogicalVolume(); // lMcp // GetMotherLogical
            for (G4int jj=0; jj<temp_0->GetNoDaughters(); jj++){
                
                (*store)[jj] = temp_0->GetDaughter(jj);
                if((*store)[jj]->GetName()=="wMcp") { //  wPixel
                    if (counter == 1) break;
                    if (counter2 == 20) break;
                    std::cout<<"############### MCP["<<counter<<"]"<<(*store)[jj]->GetName()<<std::endl;
                    auto t = (*store)[jj]->GetTranslation(); // GetObjectTranslation  GetTranslation   GetFrameTranslation
                    //std::cout<<"############### MCP translation "<<"x "<<t.x()<<" y "<<t.y()<<" z "<<t.z()<<std::endl;
                    G4LogicalVolume * temp = (*store)[jj]->GetLogicalVolume(); // lMcp // GetMotherLogical
                    //std::cout<<"###### name "<<temp->GetName()<<std::endl;
                    for (G4int jjj=0; jjj<temp->GetNoDaughters(); jjj++){
                        if (counter2 == 20) break;
                        (*store)[jjj] = temp->GetDaughter(jjj);
                        
                        auto k = (*store)[jjj]->GetTranslation(); // GetObjectTranslation GetTranslation // GetFrameTranslation
                        ...
                        if(PrtManager::Instance()->GetRunType() == 0){ // LUT generation
                            ...
                            fParticleGun->SetParticlePosition(G4ThreeVector(vectPos.x(), vectPos.y(), vectPos.z()-0.6   ));
